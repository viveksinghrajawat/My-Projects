...
def home_page(request):
    # return render(request,'newapp/try.html')
...
def create_checkout_session(request,pk):
   ...
   #Update - passing order ID in checkout to update the order object in webhook
    ...
    mode='payment',
    success_url=YOUR_DOMAIN + f'/success/{product_info.id}',
    cancel_url=YOUR_DOMAIN + f'/failed/{product_info.id}',
    )
...
def create_checkout_session_cart(request):
   ...
   #Update - passing order ID in checkout to update the order object in webhook
    ...
    mode='payment',
    success_url=YOUR_DOMAIN + f'/success_cart',
    cancel_url=YOUR_DOMAIN + '/failed_cart',
    )
    return JsonResponse({'id': session.id})
...
def webhook(request):
    ...
    if event['type'] == 'checkout.session.completed':
        ...
        #getting information of order fwindow.location.href('/success')rom session
...
def success(request,id):
    ...
    product_info = Product.objects.get(pk=id)
    try:
        last_object = Order.objects.latest('id')
        orderid=list(str(last_object.order_id))
        if orderid[len(orderid)-1] == 'Z':
            orderid[len(orderid)-1] = 'A'
            orderid.append('A')
        orderid[len(orderid)-1]=chr(ord(orderid[len(orderid)-1]) + 1)
        order_id = ''.join(orderid)
        Order.objects.create(user=request.user, product=product_info ,amount=product_info.product_price, buy_time=datetime.datetime.now(), order_id=order_id , payment_satuts= 'success')
        
    except:
        Order.objects.create(user=request.user, product=product_info ,amount=product_info.product_price, buy_time=datetime.datetime.now(), order_id=f'{request.user.id}A', payment_satuts= 'success')
        
    # res = ''.join(random.choices(string.ascii_uppercase + string.digits,k=5))
    # product_info = Product.objects.get(pk=id)
    # orderid=f'{request.user.id}{res}{request.user.id}'
    # all_data_orders=Order.objects.values_list('order_id')
    # if orderid in all_data_orders:
    #     orderid=f'{request.user.id}{res}{res}{request.user.id}'
    # Order.objects.create(user=request.user, product=product_info ,amount=product_info.product_price, buy_time=datetime.datetime.now(), order_id=orderid, payment_satuts= 'success')
...
def failed(request):
    ...
    if orderid in all_data_orders:
        orderid=f'{request.user.id}{res}{res}{request.user.id}'
    Order.objects.create(user=request.user, product=product_info ,amount=product_info.product_price, buy_time=datetime.datetime.now(), order_id=orderid, payment_satuts= 'failed')
    return redirect('myorder')
...
def success_cart(request):
    ...
    for items in cart_info:
        ...
        if orderid in all_data_orders:
            orderid=f'{request.user.id}{res}{res}{request.user.id}'
        Order.objects.create(user=request.user, product=items.product ,amount=items.product.product_price, buy_time=datetime.datetime.now(), quantity=items.quantity, order_id=orderid, payment_satuts= 'success')
    cart_info.delete()
    return redirect('myorder')

def failed_cart(request):
    cart_info = Cart.objects.filter(user=request.user)
    for items in cart_info:
        try:
            last_object = Order.objects.latest(id)
            orderid=list(last_object.order_id)
            if orderid[len(orderid)] == 'Z':
                orderid[len(orderid)] = 'A'
                orderid.append('A')
            Order.objects.create(user=request.user, product=items.product ,amount=items.product_price, buy_time=datetime.datetime.now(), quantity=items.quantity, order_id=orderid, payment_satuts= 'failed')
        
        except:
            Order.objects.create(user=request.user, product=items.product ,amount=items.product_price, buy_time=datetime.datetime.now(), quantity=items.quantity, order_id=f'{request.user.id}A', payment_satuts= 'failed')
        res = ''.join(random.choices(string.ascii_uppercase + string.digits,k=5))
        orderid = f'{request.user.id}{res}{request.user.id}'
        all_data_orders=Order.objects.values_list('order_id')
        if orderid in all_data_orders:
